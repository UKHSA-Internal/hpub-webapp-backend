...
def lambda_handler(event, context):
    """
    AWS Lambda handler that automatically deletes all users who haven't logged in
    since the threshold date (3 years ago).

    It calculates a threshold datetime (3 years ago from now) based on the last_login field,
    filters the User queryset accordingly, and then for each inactive user, it performs a hard delete.
    """
    # Define the threshold for inactivity - 3 years ago.
    threshold_date = timezone.now() - timedelta(days=3 * 365)

    # Retrieve all users whose last_login is less than the threshold.
    inactive_users = User.objects.filter(last_login__lt=threshold_date)
    results = []

    for user in inactive_users:
        result = delete_user_and_dependencies(user.user_id)
        results.append(result)
        logger.info("Deleted user: %s", user.email)

    return {
        "statusCode": 200,
        "body": f"{inactive_users.count()} users processed.",
        "results": results,
    }
