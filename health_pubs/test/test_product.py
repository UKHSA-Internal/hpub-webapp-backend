...
class DummyProduct:
    ...
    def __dict__(self):
        ...
        return {
            ...
            "update_ref": self.update_ref.__dict__ if self.update_ref else {},
        }
...
class DummyProductUpdate:
    ...
    def __dict__(self):
        return {"product_downloads": self.product_downloads}
...
# Instead of using reverse(), we define dummy URL constants.
...
class DummyRenderer:
    ...
    format = "json"
    charset = "utf-8"  # Added charset attribute

    def render(self, data, accepted_media_type=None, renderer_context=None):
        return json.dumps(data)


# --- Dummy classes for testing Product detail view ---
...
class DummyProductSerializer:
    ...
    def _serialize(self, obj):
        return {
            ...
            # For update_ref, we return its product_downloads as a dict.
            "update_ref": {
...
def test_bulk_upload_success(
    ...
    with patch.object(ProductViewSet, "permission_classes", []):
        # For viewsets, you can pass a mapping (if your view is a ViewSet) but if you get errors, simply call as_view() with no parameters.
...
def test_product_detail_view_success(mock_filter):
    ...
    # Create a dummy update with a downloads dict.
    ...
    # Bypass any permission checks by patching the view's permission_classes.
    from core.products.views import (
        ProductDetailView,
...
def test_product_delete_all_success(mock_product_all, mock_productupdate_all):
    ...
    # Import the view; adjust the import path as necessary.
    from core.products.views import ProductDeleteAll

    view = ProductDeleteAll.as_view()
    response = view(request)
    # For a JsonResponse, decode the content from bytes.
    data = json.loads(response.content.decode("utf-8"))
...
def test_product_status_update_database_error(mock_filter):
    ...
    force_authenticate(request, user=get_dummy_user(is_admin=True))
    from core.products.views import ProductStatusUpdateView  # Adjust import if needed.

    # Patch permission_classes to bypass permission checks and patch handle_error to return a DRF Response.
    with patch.object(ProductStatusUpdateView, "permission_classes", []), patch.object(
        ProductStatusUpdateView,
        "handle_error",
        side_effect=lambda err, msg, status_code: Response(
        ...
        view = ProductStatusUpdateView.as_view()
        response = view(request, product_code="P-001")
    # Set dummy renderer attributes so that response.render() can be called.
    response.accepted_renderer = DummyRenderer()
    response.accepted_media_type = "application/json"
    response.renderer_context = {}
    # Render the response so we can access its content.
    response.render()
    data = json.loads(response.content.decode("utf-8"))
    print("PRODUCT STATUS UPDATE DATABASE ERROR RESPONSE:", data)
    assert response.status_code == status.HTTP_500_INTERNAL_SERVER_ERROR
    # Convert the expected error message to string.
...
def test_product_update_view_success(mock_serializer_class, mock_filter):
    ...
    dummy_qs.order_by.return_value.first.return_value = dummy_product
    mock_filter.return_value = dummy_qs
    # Create a dummy serializer instance that simulates a successful update.
    dummy_serializer = MagicMock()
    dummy_serializer.is_valid.return_value = True
    dummy_serializer.save.return_value = dummy_product
    dummy_serializer.data = {"product_title": "Updated Product Title"}
    ...
    with patch.object(ProductUpdateView, "permission_classes", []):
        view = ProductUpdateView.as_view()
        response = view(request, product_code="P-001")
    print("PRODUCT UPDATE VIEW RESPONSE:", response.content)
    assert response.status_code == status.HTTP_200_OK
    response_data = json.loads(response.content)
    assert response_data["product_title"] == "Updated Product Title"
